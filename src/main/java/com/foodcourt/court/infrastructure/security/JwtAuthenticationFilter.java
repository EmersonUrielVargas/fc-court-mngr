...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        final String authHeader = request.getHeader(GeneralConstants.AUTH_HEADER_NAME);
        ...
        jwtToken = authHeader.replace(GeneralConstants.AUTH_TOKEN_PREFIX,"");
    ...
    private UserDetails buildUserDetails(String token, String email){
        String roleFromToken = jwtService.extractClaim(
                token,
                claims->claims.get(GeneralConstants.ROL_KEY_TOKEN, String.class)
        );
        Long userId = jwtService.extractClaim(
                token,
                claims->claims.get(GeneralConstants.USER_ID_KEY_TOKEN, Long.class)
        );
